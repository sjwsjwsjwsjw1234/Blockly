...
public class RegisterActivity extends Activity implements View.OnClickListener {
    ...
    private Button  register_countdown;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mContext = this;
        addControl();//加载控件
        //Bmob.resetDomain("http://www.codeland.waityousell.com/8/");
...
//                register_countdown.setEnabled(false);
                ...
                BmobSMS.requestSMSCode(mobile, "", new QueryListener<Integer>() {
                    @Override
                    public void done(Integer smsId, BmobException e) {
                        if (e == null) {
                            showToast(getString(R.string.smssuccess) + smsId);
                        } else {
                            showToast(getString(R.string.failed) + e.getErrorCode() + "-" + e.getMessage() + "\n");
                        }
                    }
                });
                break;
                ...
                if (mobile.isEmpty() || password.isEmpty()) {
                    ...
                    p2.signOrLogin(valiation, new  SaveListener<BmobUser>() {//valiation关于获取到验证码
                        ...
                        public void done(BmobUser bmobUser, BmobException e) {
                            if (e == null) {
                                //判断是否注册成功成功则跳转到登陆的页面
                                Intent intent_register = new Intent(RegisterActivity.this, ForumActivity.class);
                                startActivity(intent_register);
                                showToast(getString(R.string.user_success));
                            } else {
                                showToast(getString(R.string.user_failed) );
                            }
                        }
                    });
                    break;
